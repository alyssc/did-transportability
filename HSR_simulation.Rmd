...
prop_treated <- bind_cols(
  (
    df %>% 
      filter(S==1,X1==0) %>% 
      summarise(prop_noSSP_treated = mean(A)) ),
  (
    df %>% 
      filter(S==1,X1==1) %>% 
  ...
  (
    df %>% 
      filter(S==1,X2==0) %>% 
      summarise(prop_nosys_treated = mean(A)) ),
  (
    df %>% 
      filter(S==1,X2==1) %>% 
...
for(adjust1 in seq(-.3,.3,.1) ){
  new_params$beta.0= global_params$beta.0 + adjust1
  print(c(adjust1, calib_assist(new_params)[1,"P(A | S)"] ))
}
```
...
for(adjust1 in seq(-.02, .02, .01 ) ){
  ...
  print(c(adjust1, calib_assist2(new_params)[1,2],  calib_assist2(new_params)[2,2]))
}
